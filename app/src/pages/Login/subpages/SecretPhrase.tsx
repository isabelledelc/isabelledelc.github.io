      ...
      <main className="flex-1 flex flex-col px-4 sm:px-6 py-4 sm:py-6 max-w-5xl mx-auto w-full">
        ...
        {step === 'select' && (
          <div className="space-y-6 sm:space-y-8 flex-1 flex flex-col justify-between">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8 items-end pt-4 sm:pt-6">
              <div>
              ...
              <div>
                <button
                  ...
                  className={`w-full py-3.5 rounded-xl font-semibold text-white transition shadow-sm cursor-pointer text-base ${
        ...
        {step === 'confirm' && (
          <div className="space-y-6 sm:space-y-8 flex-1 flex flex-col justify-between">
            <div>
              ...
              {/* Summary Card */}
              <div className="max-w-2xl mx-auto border-2 border-[#22c55e] rounded-xl p-6 sm:p-10 flex flex-col items-center justify-center space-y-4 sm:space-y-6 bg-white min-h-[220px] sm:min-h-[280px]">
                {selectedImage && (
                  <img
                    src={selectedImage.src}
                    alt={selectedImage.alt}
                    className="h-28 sm:h-36 max-w-xs object-contain"
                  />
                )}
                <span className="text-xl sm:text-2xl font-black text-slate-800 tracking-wide text-center break-all">
                  {secretText}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-6 pt-4 sm:pt-6">
              <button
                type="button"
                onClick={() => setStep('select')}
                className="w-full sm:w-36 py-3 sm:py-2.5 rounded-xl border-2 border-[#22c55e] text-slate-800 font-bold hover:bg-emerald-50 transition cursor-pointer order-2 sm:order-1"
              >
                Edit
              </button>
              <button
                type="button"
                onClick={handleSave}
                className="w-full sm:w-48 py-3 sm:py-2.5 rounded-xl bg-[#22c55e] hover:bg-emerald-600 text-white font-bold transition shadow-sm cursor-pointer order-1 sm:order-2"
              >
                Save
              </button>
            </div>
