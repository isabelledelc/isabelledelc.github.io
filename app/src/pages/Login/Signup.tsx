...
export default function Signup() {
  ...
  return (
    <div 
      className="min-h-screen w-full flex flex-col md:flex-row font-sans transition-colors overflow-hidden"
      style={{ backgroundColor: 'var(--bg-signup-page)' }}
    >
      {/* LEFT SIDE: Grass Background Banner */}
      <div 
        className="relative w-full md:w-[42%] lg:w-[40%] min-h-[300px] md:min-h-screen bg-cover bg-center flex flex-col justify-between p-6 sm:p-10"
        style={{ backgroundImage: `url(${grassBg})` }}
      >
        {/* Subtle Overlay */}
        <div className="absolute inset-0 bg-black/10 z-0" />

        {/* Top Back Chevron Button -> Navigates to Landing Page ('/') */}
        <button
          type="button"
          onClick={() => navigate('/')}
        ...
        <div className="relative z-10 my-auto flex justify-center items-center px-4 animate-fade-in">
          <img 
            src={logoImg} 
            alt="Opus Touch Logo" 
            className="w-full max-w-[280px] sm:max-w-[320px] h-auto object-contain drop-shadow-xl hover:scale-105 transition-transform duration-300" 
          />
        </div>
      ...
      <div className="flex-1 flex flex-col justify-between p-8 sm:p-12 md:p-16 lg:p-24 bg-white dark:bg-gray-900 transition-colors">
